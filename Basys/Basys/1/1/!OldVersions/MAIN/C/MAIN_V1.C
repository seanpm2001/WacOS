...
#include <string.h>
// Start of script
// BaSYSTEM 1 - A compatibility layer for Apple System 1
...
int wacSystemSoftware() {
	continue {
		char winTitle = "BaSYS 1 Disk";
		double freeMEM = 1048576;
		double usedMEM = 0;
		break;
	}
}
int programs() {
	...
	int controlPanel() {
		...
		int sysInfo() {
			...
			int aboutBASYSOne() {
				printf("About BaSYS/1");
				printf("WacOS operating system/BASYS/1");
				printf("Version 0.01 Alpha");
				printf("2021-2021");
				return credits();
				break;
			}
			return aboutBASYSOne();
			break;
		}
		return startCPanel();
		break;
...
int desktop() {
	...
	int icons() {
		int trash() {
			double capacity = "1048576"; // Capacity in bytes (2^20, 1.048 MB)
			continue {
				// Open trash can
				return system.trash();
			}
...
int fileSystem() {
	char fsystype = "HFS";
	double volumeSize = "1048576"; // Capacity in bytes (2^20, 1.048 MB)
	int systemFolder() {
		int sysDirContents() {
			// Fonts
			#include </FONT/MONACO.TTF>
...
/* File info
* File version: 1 (2021, Wednesday, December 22nd at 3:01 pm)
